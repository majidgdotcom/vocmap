...
export const WordEntrySchema = z.object({
  ...
  lang:     z.string().max(10).default('per'),  // e.g. "per", "en", "ar", "fr"
...
export const SaveWordFamilySchema = z.object({
  ...
  words: z.array(WordEntrySchema).min(1, 'words must not be empty').max(200, 'max 200 words per family'),
...
export const BatchSaveFamiliesSchema = z.object({
  families: z.array(SaveWordFamilySchema).min(1).max(50, 'max 50 families per batch'),
});
