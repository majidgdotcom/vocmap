...
#
# Add these steps in order AFTER "Deploy keyword-service"
# and BEFORE "Build + deploy frontend".
# Each block follows the same pattern as the existing service deploys.
#
# ── word-family-service ───────────────────────────────────────────────────────
...
# ── vocabulary-service ────────────────────────────────────────────────────────
      ...
      - name: Export VOCAB_URL for frontend
        run: |
          URL=$(aws cloudformation describe-stacks \
            --stack-name vocmap-vocabulary-service-${{ env.DEPLOY_ENV }} \
            --query "Stacks[0].Outputs[?OutputKey=='VocabularyApiUrl'].OutputValue" \
            --output text)
          echo "VITE_API_VOCAB_URL=$URL" >> $GITHUB_ENV

# ── Teardown (add to your teardown section) ───────────────────────────────────
#
