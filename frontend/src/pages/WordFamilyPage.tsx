...
import {
  ...
  WordFamilyViewModel,
  getCategory,
} from '@/viewmodels/word-family.viewmodel';
...
function BatchSaveBar({ validBlocks, onSaved }: { validBlocks: JSONBlock[]; onSaved: (indices: Set<number>) => void }) {
  ...
  async function handleSave() {
    ...
    setDone(false); setPhase('families');
      ...
      try {
        ...
        if (Array.isArray(savedFamilies) && savedFamilies.length > 0) {
          await Promise.allSettled(
            savedFamilies.map((f: { familyId: string }) => saveToVocab.mutateAsync(f.familyId))
          );
        }
        setPhase('families');
      } catch (err) {
    ...
    if (failed.length === 0) {
      ...
      setShowForm(false); setTags(''); setNotes('');
    }
  }

  function handleReset() { setProgress(null); setDone(false); }
    ...
    <div style={{ border: `1.5px solid ${borderColor}`, borderRadius: 10, padding: '12px 16px', background: bgColor, marginTop: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
        ...
        {allSuccess ? (
          ...
          <span style={{ fontSize: 12, color: '#185FA5', fontWeight: 600 }}>Batch {progress!.current} / {progress!.total}…</span>
        ) : (
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            {!done && <button onClick={() => setShowForm(v => !v)} style={{ fontSize: 12, padding: '6px 18px', borderRadius: 8, background: '#185FA5', color: '#fff', border: 'none', fontWeight: 700, cursor: 'pointer' }}>{showForm ? '✕ Cancel' : '→ Save all to DynamoDB'}</button>}
          </div>
        )}
      </div>
        ...
        <div style={{ marginTop: 12 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
            <span style={{ fontSize: 11, color: '#5a7a9c' }}>
              {isPending
                ? (phase === 'vocabulary' ? `Saving to Vocabulary (batch ${progress.current + 1}/${progress.total})…` : `Saving families batch ${progress.current + 1} of ${progress.total}…`)
                : done ? (allSuccess ? '✓ Saved to families + vocabulary' : `Done — ${progress.failed.length} batch${progress.failed.length > 1 ? 'es' : ''} failed`) : ''}
            </span>
            <span style={{ fontSize: 11, fontWeight: 600, color: '#185FA5', fontFamily: 'monospace' }}>{progress.saved} / {validBlocks.length} saved · {pct}%</span>
          </div>
          <div style={{ height: 6, borderRadius: 999, background: '#dde8f5', overflow: 'hidden' }}>
            <div style={{ height: '100%', borderRadius: 999, width: `${pct}%`, background: allFailed ? '#993C1D' : partialFail ? '#854F0B' : '#185FA5', transition: 'width 0.3s ease' }} />
          </div>
          {progress.total <= 30 && (
            <div style={{ display: 'flex', gap: 3, marginTop: 6, flexWrap: 'wrap' }}>
              {Array.from({ length: progress.total }, (_, i) => {
                const isDone = i < progress.current, isFailed = progress.failed.some(f => f.startsWith(`Chunk ${i + 1}/`)), isActive = i === progress.current && isPending;
                return <div key={i} title={`Batch ${i + 1}`} style={{ width: 14, height: 14, borderRadius: 3, flexShrink: 0, background: isFailed ? '#993C1D' : isDone ? '#0F6E56' : isActive ? '#185FA5' : '#dde8f5', opacity: isActive || isDone || isFailed ? 1 : 0.5, outline: isActive ? '2px solid #185FA5' : 'none', outlineOffset: 1 }} />;
              })}
            </div>
          )}
        </div>
      ...
      {done && progress && progress.failed.length > 0 && (
        <div style={{ marginTop: 10 }}>
          {progress.failed.map((msg, i) => <div key={i} style={{ fontSize: 11, color: '#993C1D', background: '#FAECE7', padding: '4px 10px', borderRadius: 6, marginBottom: 4, fontFamily: 'monospace' }}>✗ {msg}</div>)}
          {partialFail && <div style={{ fontSize: 11, color: '#854F0B', background: '#FAEEDA', padding: '4px 10px', borderRadius: 6, marginTop: 4 }}>⚠ {progress.saved} of {validBlocks.length} saved. Click Reset to retry.</div>}
        </div>
      )}

      {showForm && !progress && (
        <div style={{ marginTop: 14, display: 'flex', flexDirection: 'column', gap: 10, borderTop: '1px solid #d5e8f8', paddingTop: 14 }}>
          <p style={{ fontSize: 11, color: '#5a7a9c', margin: 0 }}>Applied to all <strong>{validBlocks.length} families</strong> across <strong>{totalChunks} batches</strong> of {CHUNK_SIZE}.</p>
          <div>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
            <button onClick={handleSave} style={{ fontSize: 13, padding: '8px 24px', borderRadius: 8, background: '#185FA5', color: '#fff', border: 'none', fontWeight: 700, cursor: 'pointer' }}>
...
interface BatchVocabProgress { current: number; total: number; failed: string[]; }

function BatchVocabBar({ pendingFamilies, onDone }: { pendingFamilies: WordFamilyViewModel[]; onDone: () => void }) {
  ...
  async function handleRun() {
    setShowBar(true); setDone(false);
    const failed: string[] = [];
    setProgress({ current: 0, total: pendingFamilies.length, failed: [] });
    for (let i = 0; i < pendingFamilies.length; i++) {
      try { await saveToVocab.mutateAsync(pendingFamilies[i].familyId); }
      catch { failed.push(pendingFamilies[i].title); }
  ...
  return (
    <div style={{ border: `1.5px solid ${allSuccess ? '#9FE1CB' : hasFailed ? '#F5C4B3' : '#c7d2fe'}`, borderRadius: 10, padding: '12px 16px', marginBottom: 12, background: allSuccess ? '#F4FCF8' : hasFailed ? '#FDF5F3' : '#f5f3ff' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          {isPending && <span style={{ fontSize: 11, color: '#6366f1' }}>{progress!.current} / {progress!.total} done</span>}
        </div>
        {allSuccess ? (
          <span style={{ fontSize: 12, padding: '4px 14px', borderRadius: 999, background: '#E1F5EE', color: '#0F6E56', fontWeight: 700 }}>✓ All saved to Vocabulary</span>
        ) : isPending ? (
          <span style={{ fontSize: 12, color: '#6366f1', fontWeight: 600 }}>Saving…</span>
        ) : (
          <div style={{ display: 'flex', gap: 8 }}>
            {showBar && hasFailed && <button onClick={handleReset} style={{ fontSize: 12, padding: '6px 14px', borderRadius: 8, background: 'transparent', color: '#888', border: '1.5px solid #e0e0e0', cursor: 'pointer' }}>Reset</button>}
            {!showBar && <button onClick={handleRun} style={{ fontSize: 12, padding: '6px 18px', borderRadius: 8, background: '#6366f1', color: '#fff', border: 'none', fontWeight: 700, cursor: 'pointer' }}>🔤 Save all {pendingFamilies.length} to Vocabulary</button>}
          </div>
        )}
      </div>
      {showBar && progress && (
        <div style={{ marginTop: 10 }}>
          <div style={{ height: 6, borderRadius: 999, background: '#e0e7ff', overflow: 'hidden' }}>
            <div style={{ height: '100%', borderRadius: 999, width: `${pct}%`, background: hasFailed ? '#854F0B' : '#6366f1', transition: 'width 0.2s ease' }} />
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 5 }}>
            <span style={{ fontSize: 11, color: '#818cf8' }}>
              {isPending ? `Saving "${pendingFamilies[progress.current]?.title ?? '…'}"…` : done ? (allSuccess ? 'Complete' : `${progress.failed.length} failed`) : ''}
            </span>
            <span style={{ fontSize: 11, fontWeight: 600, color: '#6366f1', fontFamily: 'monospace' }}>{pct}%</span>
          </div>
        </div>
      )}
      {hasFailed && progress && (
        <div style={{ marginTop: 8 }}>
          {progress.failed.map((title, i) => <div key={i} style={{ fontSize: 11, color: '#993C1D', background: '#FAECE7', padding: '4px 10px', borderRadius: 6, marginBottom: 4, fontFamily: 'monospace' }}>✗ Failed: {title}</div>)}
        </div>
      )}
    </div>
  );
...
// ── FamilyCard — receives WordFamilyViewModel ─────────────────────────────────

function FamilyCard({
  vm, isExpanded, isConfirming,
  onToggle, onConfirm, onCancelConfirm, onDelete,
}: {
  vm: WordFamilyViewModel;
  ...
  return (
    <div style={{ border: '1px solid #d5e8d4', borderRadius: 10, overflow: 'hidden', background: '#fff' }}>
      <div style={{ display: 'flex', alignItems: 'center', padding: '10px 14px', background: '#F4FCF8', gap: 10, flexWrap: 'wrap' }}>
        ...
        {/* Category distribution — pre-computed by ViewModel */}
        {vm.categoryDistribution.map(({ category, count }) => (
          <span key={category} style={{ fontSize: 10, padding: '2px 7px', borderRadius: 999, background: CAT_BG[category] ?? '#f5f5f5', color: CAT_COLOR[category] ?? '#555', fontWeight: 600 }}>
            {category} ×{count}
          </span>
        ))}

        {vm.tags.map(tag => (
          <span key={tag} style={{ fontSize: 10, padding: '2px 9px', borderRadius: 999, background: '#EEEDFE', color: '#534AB7', fontWeight: 500 }}>{tag}</span>
        ))}

        {/* isSavedToVocabulary — pre-computed by ViewModel */}
        {vm.isSavedToVocabulary && (
          <span style={{ fontSize: 10, padding: '2px 9px', borderRadius: 999, background: '#EAF3DE', color: '#3B6D11', fontWeight: 600 }}>🔤 in vocabulary</span>
        )}

        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
          {/* savedDate — formatted by ViewModel */}
          <span style={{ fontSize: 11, color: '#bbb' }}>{vm.savedDate}</span>

          {!vm.isSavedToVocabulary && (
            <button
              onClick={() => saveToVocab.mutate(vm.familyId)}
              disabled={saveToVocab.isPending}
              title="Save to Vocabulary"
              style={{ fontSize: 11, padding: '3px 9px', borderRadius: 6, fontWeight: 600, background: saveToVocab.isPending ? '#f5f5f5' : '#eef2ff', color: saveToVocab.isPending ? '#aaa' : '#6366f1', border: '1px solid #c7d2fe', cursor: saveToVocab.isPending ? 'not-allowed' : 'pointer' }}
            >
      ...
      {isExpanded && (
        <div style={{ padding: '12px 14px', borderTop: '1px solid #e8f5e8' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {/* vm.words already have category pre-computed */}
            {vm.words.map((w, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', padding: '6px 10px', borderRadius: 6, background: '#fafafa', border: '1px solid #f0f0f0' }}>
                ...
                <span style={{ fontSize: 10, padding: '1px 6px', borderRadius: 999, background: '#f3f4f6', color: '#6b7280', fontFamily: 'monospace' }}>{w.lang}</span>
...
function SavedFamiliesPanel() {
  ...
  const displayed  = vocabFilter === 'all' ? allVMs : vocabFilter === 'inVocab' ? inVocab : notInVocab;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      {notInVocab.length > 0 && <BatchVocabBar pendingFamilies={notInVocab} onDone={() => setVocabFilter('inVocab')} />}

      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
        {([
          { key: 'all',        label: `All (${allVMs.length})` },
          ...
          <button key={f.key} onClick={() => setVocabFilter(f.key)} style={{ fontSize: 12, padding: '5px 13px', borderRadius: 999, border: '1.5px solid', borderColor: vocabFilter === f.key ? '#1a1a1a' : '#e0e0e0', background: vocabFilter === f.key ? '#1a1a1a' : 'transparent', color: vocabFilter === f.key ? '#fff' : '#888', cursor: 'pointer', fontWeight: vocabFilter === f.key ? 600 : 400 }}>
            {f.label}
          </button>
        ))}
      </div>

      {displayed.length === 0 && <div style={{ fontSize: 13, color: '#aaa', padding: '16px 0', textAlign: 'center' }}>No families match this filter.</div>}

      {displayed.map(vm => (
        <FamilyCard
          key={vm.familyId}
          vm={vm}
          isExpanded={expandedId === vm.familyId}
          isConfirming={confirmId === vm.familyId}
          onToggle={() => setExpandedId(id => id === vm.familyId ? null : vm.familyId)}
          onConfirm={() => setConfirmId(vm.familyId)}
          onCancelConfirm={() => setConfirmId(null)}
          onDelete={() => { deleteFamily.mutate(vm.familyId); setConfirmId(null); }}
        />
      ))}

      {hasNextPage && (
        <button onClick={() => fetchNextPage()} disabled={isFetchingNextPage} style={{ marginTop: 8, padding: '10px 0', width: '100%', borderRadius: 10, border: '1.5px dashed #c7d2fe', background: isFetchingNextPage ? '#f5f5f5' : '#fff', color: isFetchingNextPage ? '#aaa' : '#6366f1', fontSize: 13, fontWeight: 600, cursor: isFetchingNextPage ? 'not-allowed' : 'pointer' }}>
          {isFetchingNextPage ? 'Loading…' : 'Load next 50 →'}
        </button>
      )}
      {!hasNextPage && allVMs.length > 50 && (
        <div style={{ textAlign: 'center', fontSize: 11, color: '#bbb', padding: '10px 0' }}>All {allVMs.length} families loaded</div>
      )}
    </div>
  );
}
...
export const WordFamilyPage: React.FC = () => {
  ...
  const handleAnalyze = useCallback(() => {
    ...
    setHasRun(true); setFilter('all'); setSavedIndices(new Set());
  }, [inputText]);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#f9fafb' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '24px 20px', maxWidth: 820, width: '100%', margin: '0 auto', boxSizing: 'border-box' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
          <div>
            <h1 style={{ fontSize: 20, fontWeight: 700, margin: 0, fontFamily: 'monospace', letterSpacing: '-0.03em' }}>📚 Word Family Validator</h1>
            <p style={{ fontSize: 12, color: '#888', margin: '3px 0 0' }}>Paste text · validate JSON blocks · save to DynamoDB</p>
          </div>
          <div style={{ display: 'flex', borderRadius: 8, overflow: 'hidden', border: '1.5px solid #e0e0e0' }}>
            {(['analyzer', 'saved'] as const).map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)} style={{ fontSize: 12, padding: '7px 16px', cursor: 'pointer', fontWeight: activeTab === tab ? 700 : 400, border: 'none', background: activeTab === tab ? '#1a1a1a' : 'transparent', color: activeTab === tab ? '#fff' : '#888', borderRight: tab === 'analyzer' ? '1px solid #e0e0e0' : 'none' }}>
        ...
        {activeTab === 'analyzer' && (
          <>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 12 }}>
              <button onClick={handleAnalyze} disabled={!inputText.trim()} style={{ padding: '9px 22px', borderRadius: 8, background: inputText.trim() ? '#1a1a1a' : '#ccc', color: '#fff', border: 'none', fontSize: 13, fontWeight: 700, cursor: inputText.trim() ? 'pointer' : 'not-allowed' }}>Analyze</button>
              {hasRun && <button onClick={handleClear} style={{ padding: '9px 18px', borderRadius: 8, background: 'transparent', color: '#888', border: '1.5px solid #e0e0e0', fontSize: 13, cursor: 'pointer' }}>Clear</button>}
            </div>
            ...
            {hasRun && (
              <div style={{ marginTop: 24 }}>
                ...
                {total === 0 ? (
                  <div style={{ textAlign: 'center', padding: '32px', color: '#aaa', fontSize: 13, border: '1.5px dashed #eee', borderRadius: 10 }}>No JSON array blocks found in this text.</div>
                ) : (
                  <>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                      {filtered.map(block => <BlockCard key={block.index} block={block} isSaved={savedIndices.has(block.index)} />)}
                    </div>
                    <BatchSaveBar validBlocks={blocks.filter(b => b.isValid)} onSaved={setSavedIndices} />
                  </>
                )}
              </div>
            )}
