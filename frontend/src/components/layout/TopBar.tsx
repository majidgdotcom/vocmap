...
interface TopBarProps { onNewTodo: () => void; }
    ...
    <header style={barStyle}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
        ...
        <nav style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          <NavLink to="/" end style={navStyle}>Todos</NavLink>
          <NavLink to="/word-families" style={navStyle}>📝 Word Families</NavLink>
          <NavLink to="/vocabulary" style={navStyle}>🔤 Vocabulary</NavLink>
        </nav>
        {activeKeywordLabel && (
          <span style={filterBadgeStyle}>🏷 {activeKeywordLabel}</span>
        )}
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <button onClick={onNewTodo} style={newBtnStyle}>+ New Todo</button>
        <span style={{ fontSize: 13, color: '#6b7280' }}>
          {user?.signInDetails?.loginId ?? user?.username}
        </span>
        <button onClick={() => signOut()} disabled={isSigningOut} style={signOutBtnStyle}>
...
const barStyle: React.CSSProperties = {
  ...
  borderBottom: '1px solid #e5e7eb', background: '#fff', flexShrink: 0, zIndex: 10,
};
...
const filterBadgeStyle: React.CSSProperties = {
  ...
  background: '#eef2ff', color: '#6366f1', border: '1px solid #c7d2fe', borderRadius: 9999,
};
