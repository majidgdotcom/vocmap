...
import { ConflictError, NotFoundError } from '../application/todo.use-cases';

type HandlerFn = (event: APIGatewayProxyEvent) => Promise<APIGatewayProxyResult>;

/**
 * Wraps a handler with consistent error handling and CORS headers.
 */
  ...
  return async (event) => {
    try {
      ...
      if (err instanceof ConflictError) {
        return response(409, { success: false, error: err.message, code: 'CONFLICT' });
      }
...
/** Extract verified userId from Cognito JWT claims. */
